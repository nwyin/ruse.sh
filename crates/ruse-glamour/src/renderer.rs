...
impl TermRenderer {
    ...
    pub fn render(&self, markdown: &str) -> String {
        ...
        if self.word_wrap > 0 {
            ruse_ansi::wordwrap(&out, self.word_wrap)
        } else {
            out
        }
    }
...
mod tests {
    ...
    fn test_render_nonempty() {
        ...
        assert!(ruse_ansi::strip_ansi(&output).contains("Hello"));
        assert!(ruse_ansi::strip_ansi(&output).contains("World"));
    }
