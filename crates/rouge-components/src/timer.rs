...
impl Timer {
    ...
    fn tick_cmd(&self) -> Cmd {
        ...
        let id = self.id;
        let tag = self.tag;
        rouge_runtime::tick(dur, move |_| Msg::custom(TimerTickMsg { id, tag }))
    }

    fn timeout_cmd(&self) -> Cmd {
        let id = self.id;
        rouge_runtime::cmd(move || Msg::custom(TimerTimeoutMsg { id }))
    }

    fn start_stop_cmd(&self, running: bool) -> Cmd {
        let id = self.id;
        rouge_runtime::cmd(move || Msg::custom(TimerStartStopMsg { id, running }))
    }
}
